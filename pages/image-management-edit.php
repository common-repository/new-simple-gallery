...
else
{
	...
	$form = array(
		'nsg_path' => $data['nsg_path'],
		'nsg_link' => $data['nsg_link'],
		'nsg_target' => $data['nsg_target'],
		'nsg_title' => $data['nsg_title'],
		'nsg_order' => $data['nsg_order'],
		'nsg_status' => $data['nsg_status'],
		'nsg_type' => $data['nsg_type']
	);
}
...
if (isset($_POST['nsg_form_submit']) && $_POST['nsg_form_submit'] == 'yes')
{
	...
	check_admin_referer('nsg_form_edit');
	...
	{	
		$sSql = $wpdb->prepare(
				...
				WHERE nsg_id = %d
				LIMIT 1",
				array($form['nsg_path'], $form['nsg_link'], $form['nsg_target'], $form['nsg_title'], $form['nsg_order'], $form['nsg_status'], $form['nsg_type'], $did)
			);
		$wpdb->query($sSql);
		
		$nsg_success = __('Image details was successfully updated.', 'new-simple-gallery');
	}
}
...
if ($nsg_error_found == FALSE && strlen($nsg_success) > 0)
{
	...
	<div class="updated fade">
		<p><strong><?php echo $nsg_success; ?> 
		<a href="<?php echo WP_NSG_ADMIN_URL; ?>"><?php _e('Click here', 'new-simple-gallery'); ?></a> <?php _e('to view the details', 'new-simple-gallery'); ?></strong></p>
	</div>
	<?php
}
?>
...
<div class="form-wrap">
	...
	<form name="nsg_form" method="post" action="#" onsubmit="return nsg_submit()"  >
      <h3><?php _e('Update image details', 'new-simple-gallery'); ?></h3>
      <label for="tag-image"><?php _e('Enter image path', 'new-simple-gallery'); ?></label>
      <input name="nsg_path" type="text" id="nsg_path" value="<?php echo $form['nsg_path']; ?>" size="90" />
	  <input type="button" name="upload-btn" id="upload-btn" class="button-secondary" value="Upload Image">
      <p><?php _e('Select and upload your image.', 'new-simple-gallery'); ?></p>
      <label for="tag-link"><?php _e('Enter target link', 'new-simple-gallery'); ?></label>
      <input name="nsg_link" type="text" id="nsg_link" value="<?php echo $form['nsg_link']; ?>" size="90" />
      ...
      <select name="nsg_target" id="nsg_target">
        <option value='_blank' <?php if($form['nsg_target']=='_blank') { echo 'selected' ; } ?>>_blank</option>
        <option value='_parent' <?php if($form['nsg_target']=='_parent') { echo 'selected' ; } ?>>_parent</option>
        <option value='_self' <?php if($form['nsg_target']=='_self') { echo 'selected' ; } ?>>_self</option>
        <option value='_new' <?php if($form['nsg_target']=='_new') { echo 'selected' ; } ?>>_new</option>
      </select>
      ...
      <input name="nsg_title" type="text" id="nsg_title" value="<?php echo $form['nsg_title']; ?>" size="90" />
	  ...
	  <select name="nsg_type" id="nsg_type">
			<option value='Group1' <?php if($form['nsg_type']=='Group1') { echo 'selected' ; } ?>>Group1</option>
			<option value='Group2' <?php if($form['nsg_type']=='Group2') { echo 'selected' ; } ?>>Group2</option>
			<option value='Group3' <?php if($form['nsg_type']=='Group3') { echo 'selected' ; } ?>>Group3</option>
			<option value='Group4' <?php if($form['nsg_type']=='Group4') { echo 'selected' ; } ?>>Group4</option>
			<option value='Group5' <?php if($form['nsg_type']=='Group5') { echo 'selected' ; } ?>>Group5</option>
			<option value='Group6' <?php if($form['nsg_type']=='Group6') { echo 'selected' ; } ?>>Group6</option>
			<option value='Group7' <?php if($form['nsg_type']=='Group7') { echo 'selected' ; } ?>>Group7</option>
			<option value='Group8' <?php if($form['nsg_type']=='Group8') { echo 'selected' ; } ?>>Group8</option>
			<option value='Group9' <?php if($form['nsg_type']=='Group9') { echo 'selected' ; } ?>>Group9</option>
		</select>
      ...
      <select name="nsg_status" id="nsg_status">
        <option value='YES' <?php if($form['nsg_status']=='YES') { echo 'selected' ; } ?>>Yes</option>
        <option value='NO' <?php if($form['nsg_status']=='NO') { echo 'selected' ; } ?>>No</option>
      </select>
      ...
      <input name="nsg_order" type="text" id="nsg_order" size="10" value="<?php echo $form['nsg_order']; ?>" maxlength="3" />
	  ...
	  <?php wp_nonce_field('nsg_form_edit'); ?>
    </form>
</div>
</div>